...
const valueIcons = [Target, Award, Zap]

function StatCounter({ stat, delay }: { stat: { value: number; suffix: string; decimals: number; label: string }; delay: number }) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center text-center">
      <div className="font-display text-5xl md:text-6xl font-black text-white mb-2 tabular-nums tracking-tight">
        {inView ? (
          <CountUp end={stat.value} duration={2.5} delay={delay} decimals={stat.decimals} suffix={stat.suffix} useEasing />
        ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center mb-16 md:mb-32">
          ...
          >
            <div className="space-y-6">
              ...
              <p className="text-white/65 font-light leading-relaxed text-base md:text-lg">{t.about.p1}</p>
              <p className="text-white/55 font-light leading-relaxed text-sm md:text-base">{t.about.p2}</p>
                  ...
                  return (
                    <div key={v.title} className="flex items-start gap-4 group">
                      <div className="w-10 h-10 rounded-sm border border-white/08 flex items-center justify-center flex-shrink-0 group-hover:border-admato-cyan/30 transition-colors duration-300">
                        <Icon size={16} className="text-white/50 group-hover:text-admato-cyan transition-colors duration-300" />
                      </div>
                      <div>
          ...
          >
            ...
            >
              ...
              <div className="font-mono text-xs text-white/35 tracking-[0.2em] uppercase">{t.about.experience_badge}</div>
            </motion.div>
